...
class RoamMikadoFramework {
	...
	private function __construct() {
		$this->mkdOptions         = RoamMikadoOptions::get_instance();
		$this->mkdMetaBoxes       = RoamMikadoMetaBoxes::get_instance();
		$this->mkdTaxonomyOptions = RoamMikadoTaxonomyOptions::get_instance();
	}
	...
	public function setSkin( RoamMikadoSkinAbstract $skinObject ) {
		$this->skin = $skinObject;
	}
}

/**
 * Class RoamMikadoSkinManager
 *
...
class RoamMikadoSkinManager {
	...
	/**
	 * @see RoamMikadoSkinManager::setSkin()
	 */
	...
	private function setSkin( $skinName = 'mikado' ) {
		if ( $skinName !== '' ) {
			if ( file_exists( get_template_directory() . '/framework/admin/skins/' . $skinName . '/skin.php' ) ) {
				...
				$skinName = ucfirst( $skinName ) . esc_html__( 'Skin', 'roam' );
...
/**
 * Class RoamMikadoSkinAbstract
 *
 * Abstract class that each skin class must extend
 */
abstract class RoamMikadoSkinAbstract {
	/**
	 * @var string
	 */
	protected $skinName;
	...
	public function loadTemplatePart( $template, $params = array() ) {
		...
		if ( $template !== '' ) {
			include( roam_mikado_find_template_path( 'framework/admin/skins/' . $this->skinName . '/templates/' . $template . '.php' ) );
		}
	}
	...
	public function setShortcodeJSParams() { ?>
		<script>
			window.mkdSCIcon = '<?php echo roam_mikado_get_skin_uri() . '/assets/img/admin-logo-icon.png'; ?>';
			window.mkdSCLabel = '<?php echo esc_html( ucfirst( $this->skinName ) ); ?> <?php esc_html_e( 'Shortcodes', 'roam' ) ?>';
		</script>
	<?php }
...
class RoamMikadoOptions {
	...
	public function getOptionValue( $key ) {
		...
		if ( array_key_exists( $key, $roam_mikado_options ) ) {
			return $roam_mikado_options[ $key ];
...
/*
   Class: RoamMikadoAdminPage
   A class that initializes Mikado Admin Page
*/
class RoamMikadoAdminPage implements iRoamMikadoLayoutNode {
	public $layout;
	private $factory;
	public $slug;
	public $title;
	...
	function __construct( $slug = "", $title_label = "", $icon = "" ) {
		$this->layout  = array();
		$this->factory = new RoamMikadoFieldFactory();
		$this->slug    = $slug;
	...
	public function renderChild( iRoamMikadoRender $child ) {
		$child->render( $this->factory );
	}
}

/*
   Class: RoamMikadoMetaBoxes
...
/*
   Class: RoamMikadoMetaBox
   A class that initializes Mikado Meta Box
*/
class RoamMikadoMetaBox implements iRoamMikadoLayoutNode {
	public $layout;
	private $factory;
	public $scope;
	public $title;
	...
	public function setTitle( $label ) {
		...
		return $roam_mikado_Framework->getSkin()->getSkinLabel() . ' ' . $label;
	}
}

/*
   Class: RoamMikadoTaxonomyOptions
   A class that initializes RoamMikado Taxonomy Options
*/
class RoamMikadoTaxonomyOptions {
	private static $instance;
...
/*
   Class: RoamMikadoTaxonomyOption
   A class that initializes RoamMikado Taxonomy Option
*/
class RoamMikadoTaxonomyOption implements iRoamMikadoLayoutNode {
	public $layout;
	private $factory;
	public $scope;
	...
	function __construct( $scope = "" ) {
		$this->layout  = array();
		$this->factory = new RoamMikadoTaxonomyFieldFactory();
		$this->scope   = $scope;
	}
...
if ( ! function_exists( 'roam_mikado_init_framework_variable' ) ) {
	function roam_mikado_init_framework_variable() {
		global $roam_mikado_Framework;
		
		$roam_mikado_Framework = RoamMikadoFramework::get_instance();
		$mkdSkinManager                 = new RoamMikadoSkinManager();
		$roam_mikado_Framework->setSkin( $mkdSkinManager->getSkin() );
	}
	
	add_action( 'roam_mikado_before_options_map', 'roam_mikado_init_framework_variable' );
}
?>