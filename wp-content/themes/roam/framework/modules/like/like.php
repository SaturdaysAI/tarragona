...
class RoamMikadoLike {
	private static $instance;
	
	private function __construct() {
		add_action( 'wp_ajax_roam_mikado_like', array( $this, 'ajax' ) );
		add_action( 'wp_ajax_nopriv_roam_mikado_like', array( $this, 'ajax' ) );
	}
	...
	public function add_like() {
		...
		$title       = esc_html__( 'Like this', 'roam' );
		
		if ( isset( $_COOKIE[ 'mkdf-like_' . $post->ID ] ) ) {
			$class = 'mkdf-like liked';
			$title = esc_html__( 'You already like this!', 'roam' );
		}
...
if ( ! function_exists( 'roam_mikado_create_like' ) ) {
	function roam_mikado_create_like() {
		RoamMikadoLike::get_instance();
	}
	
	add_action( 'after_setup_theme', 'roam_mikado_create_like' );
}