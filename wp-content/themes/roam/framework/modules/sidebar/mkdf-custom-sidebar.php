 ...
 */
if ( ! class_exists( 'RoamMikadoSidebar' ) ) {
	class RoamMikadoSidebar {
		var $sidebars = array();
		var $stored = "";
		...
		function __construct() {
			...
			$this->title  = esc_html__( 'Custom Widget Area', 'roam' );
		...
		function load_assets() {
			...
			wp_enqueue_script( 'roam-sidebar', MIKADO_ROOT . '/framework/admin/assets/js/mkdf-sidebar.js' );
			wp_enqueue_style( 'roam-sidebar', MIKADO_ROOT . '/framework/admin/assets/css/mkdf-sidebar.css' );
		}
		...
		function template_add_widget_field() {
			...
			echo "\n    <span class='input_wrap'><input type='text' value='' placeholder = '" . esc_html__('Enter Name of the new Widget Area', 'roam') . "' name='mkdf-add-widget' /></span>";
			echo "\n    <input class='button' type='submit' value='" . esc_html__('Add Widget Area', 'roam') . "' />";
			echo "\n    " . $nonce;
			echo "\n  </form>";
			echo "\n</script>\n";
		}
		...
		function register_custom_sidebars() {
			...
			$args = apply_filters( 'roam_mikado_custom_widget_args', $args );
