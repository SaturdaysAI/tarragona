...
wp_enqueue_script( 'wpb-composer-front-js' );
...
if ( $has_video_bg ) {
	...
	wp_enqueue_script( 'vc-youtube-iframe-api-js' );
}

if ( ! empty( $parallax ) ) {
	wp_enqueue_script( 'vc-jquery-skrollr-js' );
...
if($row_content_width === 'grid'){
	$mkdf_row_wrapper_start .= '<div class="mkdf-row-grid-section-wrapper '.esc_attr($grid_row_classes).'" '.esc_attr($grid_row_datas).' ' . roam_mikado_get_inline_style( $mkdf_grid_row_styles ) . '><div class="mkdf-row-grid-section">';
