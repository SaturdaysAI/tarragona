				...
				if ( $can_submit ) {
					if ( ! empty( $field_data_array ) && empty( $submit_errors ) ) {
						...
						if ( $prevent_store || $entry->save() ) {
							...
							if ( ( isset( $setting['submission-behaviour'] ) && ( 'behaviour-thankyou' === $setting['submission-behaviour'] || 'behaviour-hide' === $setting['submission-behaviour'] ) ) || ( isset( $response['newtab'] ) && 'newtab_thankyou' === $response['newtab'] ) ) {
