	...
	public function render_form_header() {
		...
		<?php if ( isset( $this->model->settings['quiz_feat_image'] ) && ! empty( $this->model->settings['quiz_feat_image'] ) ): ?>
			<figure class="forminator-quiz--image">
				<img src="<?php echo esc_html( $this->model->settings['quiz_feat_image'] ); ?>" alt="<?php echo esc_html( forminator_get_form_name( $this->get_module_id(), $this->get_form_type() ) ); ?>">
			</figure>
		<?php endif; ?>

		<?php if ( isset( $this->model->settings['quiz_description'] ) && ! empty( $this->model->settings['quiz_description'] ) ):
            $content = forminator_replace_variables( $this->model->settings['quiz_description'], $this->get_module_id() );
